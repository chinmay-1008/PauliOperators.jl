...
#
#     sbatch submit.sh adaptive_smfd_4x5_reference.jl
#
...
#     results/adaptive_smfd_4x5_reference.jld2
...
# No dense cross-check is possible here: a 2^20 x 2^20 dense Hamiltonian is
# 8.8 TB. The Lanczos settings above are validated by the 3 x 4 job, which
# runs both routes and asserts they agree.
...
# -----------------------------------------------------------------------------
# 6. The 3 x 4 greedy schedule replayed on this lattice
#
# The transfer question: does the adaptive (vx, vy, vz) path calibrated on 3 x 4
# still work at 4 x 5? The schedule is read from the 3 x 4 job's own output when
# that file is present, and otherwise falls back to the path recorded in
# adaptive_smfd_3x4_bloch_vector_reference.ipynb. The two are the same numbers;
# the job says which it used.
# -----------------------------------------------------------------------------

notebook_3x4_vx = [-0.2, 0.2, 0.4, -0.2, -0.6, 0.4, 0.0, -0.4, 0.0, 0.8, -0.8, -1.0, -0.6, -0.6, -0.4, -0.8, 0.6, -0.8, -0.4, -0.6]
notebook_3x4_vy = [-0.2, 0.4, -0.8, -0.6, 0.0, 0.8, 0.0, 0.4, -0.4, 0.0, 0.6, 0.0, -0.2, 0.0, 0.6, 0.0, 0.8, 0.0, 0.4, 0.2]
...
if isfile(schedule_file)
    ...
    transfer_vx = Vector{Float64}(schedule_data["greedy_vx"])
    transfer_vy = Vector{Float64}(schedule_data["greedy_vy"])
    transfer_vz = Vector{Float64}(schedule_data["greedy_vz"])
    schedule_source = "3 x 4 job output"
    announce("3 x 4 schedule read from $(schedule_file)")

    if transfer_vx == notebook_3x4_vx &&
       transfer_vy == notebook_3x4_vy &&
       transfer_vz == notebook_3x4_vz
        announce("  matches the notebook path exactly")
    else
        announce("  NOTE: differs from the notebook path; using the job output")
    end
else
    transfer_vx = notebook_3x4_vx
    transfer_vy = notebook_3x4_vy
    transfer_vz = notebook_3x4_vz
    schedule_source = "adaptive_smfd_3x4_bloch_vector_reference.ipynb"
    announce("3 x 4 job output not found at $(schedule_file)")
    announce("  falling back to the notebook path")
end

@assert length(transfer_vx) == n_steps
@assert length(transfer_vy) == n_steps
@assert length(transfer_vz) == n_steps

transfer_norm = sqrt.(transfer_vx .^ 2 .+ transfer_vy .^ 2 .+ transfer_vz .^ 2)
@assert maximum(transfer_norm) <= 1 + 1e-12

transfer_strategies = [
    smfd_strategy(
        ProductBlochReference(N, transfer_vx[step], transfer_vy[step], transfer_vz[step]),
    )
    for step in 1:n_steps
]

announce("")
announce("Replaying the 3 x 4 greedy schedule ...")
transfer_timing = @timed propagate_strategies(transfer_strategies)
transfer = transfer_timing.value

@printf("  done in %.3f s; transferred RMSE = %.6e\n", transfer_timing.time, curve_rmse(transfer.curve))
flush(stdout)

saved["transfer_curve"] = transfer.curve
saved["transfer_term_counts"] = transfer.term_counts
saved["transfer_vx"] = transfer_vx
saved["transfer_vy"] = transfer_vy
saved["transfer_vz"] = transfer_vz
saved["transfer_norm"] = transfer_norm
saved["transfer_rmse"] = curve_rmse(transfer.curve)
saved["transfer_source"] = schedule_source
checkpoint!()
...
announce("")
announce("4 x 5 summary")

for (label, curve) in [
    ("greedy Cartesian (4 x 5)", greedy.curve),
    ("3 x 4 greedy schedule replayed", transfer.curve),
