...
@testset "Mean-field factorization" begin
    ...
    @testset "KetSum product reference matches Ket factorization" begin
        N = 5
        ψ = Ket(N, Int128(0b10101))
        ψs = KetSum(ψ; T=ComplexF64)

        for pb_str in ("ZZIZI", "XZZII", "YZZZI", "XYZIZ")
            pb = PauliBasis(pb_str)
            for k in 0:weight(pb)
                mf_ket = mean_field_factorize(pb, 0.7 + 0.2im, ψ, k)
                mf_sum = mean_field_factorize(pb, 0.7 + 0.2im, ψs, k)
                @test isapprox(mf_sum, mf_ket; atol=1e-12)
            end
        end
    end

    @testset "KetSum single-site X expectation factorizes XZ" begin
        N = 2
        ψ = KetSum(N, T=ComplexF64)
        ψ[Ket(N, 0b00)] = inv(sqrt(2)) + 0im
        ψ[Ket(N, 0b01)] = inv(sqrt(2)) + 0im  # |+0⟩ in this bit ordering

        mf = mean_field_factorize(PauliBasis("XZ"), 1.0 + 0im, ψ, 1)

        expected = PauliSum(N, ComplexF64)
        expected[PauliBasis("XI")] = 1.0 + 0im
        expected[PauliBasis("IZ")] = 1.0 + 0im
        expected[PauliBasis("II")] = -1.0 + 0im

        @test isapprox(mf, expected; atol=1e-12)
        @test expectation_value(mf, ψ) ≈ expectation_value(PauliSum{N,ComplexF64}(PauliBasis("XZ") => 1.0 + 0im), ψ)
    end

    @testset "KetSum GHZ reference is single-site mean field" begin
        N = 2
        ψ = KetSum(N, T=ComplexF64)
        ψ[Ket(N, 0b00)] = inv(sqrt(2)) + 0im
        ψ[Ket(N, 0b11)] = inv(sqrt(2)) + 0im

        O = PauliSum{N,ComplexF64}(PauliBasis("ZZ") => 1.0 + 0im)
        mf_low = mean_field_factorize(PauliBasis("ZZ"), 1.0 + 0im, ψ, 1)
        mf_full = mean_field_factorize(PauliBasis("ZZ"), 1.0 + 0im, ψ, 2)

        @test length(mf_low) == 0
        @test expectation_value(O, ψ) ≈ 1.0 + 0im
        @test expectation_value(mf_low, ψ) ≈ 0.0 + 0im
        @test isapprox(mf_full, O; atol=1e-12)
    end

    @testset "MeanFieldTruncation strategy accepts KetSum reference" begin
        N = 2
        ψ = KetSum(N, T=ComplexF64)
        ψ[Ket(N, 0b00)] = inv(sqrt(2)) + 0im
        ψ[Ket(N, 0b01)] = inv(sqrt(2)) + 0im

        O = PauliSum(N, ComplexF64)
        O[PauliBasis("XZ")] = 1.0 + 0im
        truncate!(O, MeanFieldTruncation(1, ψ))
        ...
        expected[PauliBasis("IZ")] = 1.0 + 0im
        expected[PauliBasis("II")] = -1.0 + 0im
        @test isapprox(O, expected; atol=1e-12)
    end
