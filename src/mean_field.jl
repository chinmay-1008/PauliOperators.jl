...
"""
    mean_field_factorize(pb::PauliBasis{N}, c,
                         reference::ProductDensityReference{N}, k::Int)

Order-`k` mean-field factorization around a uniform diagonal product-density
reference. All `X` and `Y` sites are unavoidable fluctuations, while every
pure-`Z` site has the common mean `reference.magnetization`.
"""
function mean_field_factorize(pb::PauliBasis{N}, c::T,
    ...
    for t in 0:min(budget, n_z)
        ...
        _foreach_combination(n_z, t) do T_idx
            T_mask = Int128(0)
            for i in T_idx
                T_mask |= _bitmask(z_pos[i])
            end
            pb_new = PauliBasis{N}(y_z_mask | T_mask, pb.x)
            _add_term!(result, pb_new, R(c) * mean_rest * f)
        end
    end
...
@inline function _product_bloch_mean(
    pb::PauliBasis,
    q::Int,
...
"""
    mean_field_factorize(pb, c, reference::ProductBlochReference, k)

Order-`k` mean-field factorization around a uniform product reference with
arbitrary local Bloch vector. Unlike `ProductDensityReference`, all three local
Pauli means can contribute.
"""
function mean_field_factorize(
    ...
    positions = get_on_bits(pb.z | pb.x)
    means = Vector{S}(undef, n)
    for (index, q) in enumerate(positions)
        means[index] = _product_bloch_mean(pb, q, reference)
    end

    for t in 0:min(k, n)
        factor = _partial_alt_binom(n - t, k - t)
        iszero(factor) && continue
        _foreach_combination(n, t) do kept_indices
            kept_mask = Int128(0)
            mean_rest = one(R)
            next_kept = 1
            for index in 1:n
                if next_kept <= length(kept_indices) &&
                   kept_indices[next_kept] == index
                    kept_mask |= _bitmask(positions[index])
                    next_kept += 1
                else
                    mean_rest *= means[index]
                end
            end

            contribution = R(c) * mean_rest * factor
            iszero(contribution) && return
            projected = PauliBasis{N}(pb.z & kept_mask, pb.x & kept_mask)
            _add_term!(result, projected, contribution)
        end
    end
...
function mean_field_factorize!(O::PauliSum{N,T},
                               reference::ProductDensityReference{N},
                               k::Int) where {N,T}
    high = [pb for (pb, _) in O if weight(pb) > k]
    for pb in high
        c = pop!(O, pb)
        sum!(O, mean_field_factorize(pb, c, reference, k))
    end
    return O
end
